...
Form-factor: mobile / web / desktop / multi-surface must resolve before IA closes. Named-protagonist journeys often derive it (Pary on iPad implies an iPad surface; Skeeter on Android adds a multi-surface need); when journeys don't disambiguate, probe. The settled form-factor also fixes the capture resolution mocks export at (`{workflow.capture_profiles}`, `references/finish-handoff.md`).
...
- **Spines distilled.** Subagent reads `.memlog.md`, `.working/`, `imports/`, sources; produces `DESIGN.md` against `## The DESIGN.md spine` + `{workflow.design_md_examples}` and `EXPERIENCE.md` against `## The EXPERIENCE.md spine` + `{workflow.experience_md_examples}`. Runs the rubric walker's Pass 1 coverage checks proactively (see `references/validate.md`). Surface gaps; never invent. Capture the **finish bar** into DESIGN.md §Do's and Don'ts — what separates a finished screen from a rough build in *this* art direction, in the user's own rejection language; elicit it if the run never produced one (`references/finish-handoff.md`).
...
- **Key-screen mocks rendered.** Key-screens tool → `.working/` for surfaces where layout drives behavior or anchors visual language. Every mock also exports a PNG at the capture resolution its form-factor implies — HTML stays the editable token-driven source, the PNG is its vision-readable twin. Keep the exports; a render thrown away after a self-check is a lost deliverable.
- **Mock coverage confirmed.** Walk every IA surface; classify *mocked* vs *spine-only*. Ask: *"These will be built from spine tables alone — any need a visual reference?"* — say plainly that only a captured surface can be judged against the real build later. Render more if named; log spine-only choices as `capture: none` + reason.
- **Layout extracted, artifacts promoted.** Distill subagent re-reads each `.working/` and `imports/` artifact; lifts visual decisions into DESIGN.md and behavioral decisions into EXPERIENCE.md. Promote `.working/` keepers to `mockups/` (HTML + its PNG capture, numbered per surface — a new revision takes the next number, an approved pair is never overwritten) or `wireframes/` (Excalidraw); imports stay. Write `SURFACES.md` (per surface: version, approval, capture path, element inventory + elements that must not leak from other surfaces) and `DIVERGENCES.md` (per surface: where the mock deliberately differs from what the product will render, why, and what a reviewer must therefore not report). Shapes and capture mechanics: `references/finish-handoff.md`. Inline relative links at relevant spine sections, citing versioned filenames; state spines-win-on-conflict once.
- **Polished, handed off, closed.** Apply `{workflow.doc_standards}` in order. Stamp approval **per surface** in `SURFACES.md` as the user signs each off; DESIGN.md frontmatter `approval` covers the art direction only, so a later edit reopens one surface rather than the package. Execute `{workflow.external_handoffs}`; surface URLs. Set both files' `status: final`, `updated: {date}`. Log finalization via `uv run {project-root}/_bmad/scripts/memlog.py append --workspace {doc_workspace} --type event --text "spines finalized"`. Share paths. Common next: `bmad-architecture`, `bmad-create-epics-and-stories`, `bmad-build`. Run `{workflow.on_complete}`.
