...
- **Customization value**: a key from the skill's own `customize.toml`, in its `[workflow]` table (most skills) or `[agent]` table (agent skills), layered with `_bmad/custom/<skill-name>.pf.toml` (generated by PrototypeFramework), `<skill-name>.toml` and `.user.toml`.
