...
**On a `kind: finish` story, weigh the lenses accordingly.** Its diff is mostly design-token values and binary assets, so the code lenses have little to read and their silence is worth little: a clean lens sweep is not evidence that the surface looks right. The load-bearing check on a finish story is step-03's Visual Contract Audit, and a finish story whose audit did not run is never treated as reviewed no matter how many lenses came back green.
